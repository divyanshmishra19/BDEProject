...
object WordCount {
  def main(args: Array[String]): Unit = {
    ...
    val words = splitOperation(lines.asInstanceOf[ProvenanceReceiverInputDStream[TaintString]])

    val mapFunc = (x: TaintString) => {
      if(x.substring(0,TaintInt(1)).equals("s"))
        (x, 1000)
      else
        (x, 1)
    }

    val mapOperation = MapOperation[TaintString, (TaintString, Int)]("x => if(x.substring(0,1).equalsIgnoreCase(\"s\")) (x, 1000) else (x, 1)", mapFunc)
    ...
    val stateDstream = updateStateByKeyOperation(wordDstream.asInstanceOf[ProvenanceDStream[(String, Int)]]);
